...
const removeColumns = (file, country) => {
   return file.map(d => {
        return [d['video_id'], d['category_id'],
                d['publish_time'], d['views'], d['likes'], d['comment_count'],d['trending_date'],d['dislikes'] , d['thumbnail_link'],d['title'],
                country]
    })
}
...
const saveFormattedVideoData = (formatVideoData) => {
    let csvContent = ""
    csvContent = csvContent + "video_id,category_id,publish_time,views,likes,comment_count,trending_date,dislikes,thumbnail_link,title,country\n"
